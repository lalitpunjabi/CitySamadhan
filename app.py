...
class Complaint(db.Model):
    ...
    similar_complaints = db.relationship(
        'Complaint',
        backref=db.backref('parent', remote_side=[id]),
        lazy='dynamic'
    )
    votes = db.relationship('Vote', backref='complaint', lazy=True)
    responses = db.relationship('AuthorityResponse', backref='complaint', lazy=True)
    reports = db.relationship('Report', backref='complaint', lazy=True)
...
def find_similar_complaints(latitude, longitude, title, description, city):
    ...
    nearby_complaints = Complaint.query.filter(
        ...
        Complaint.parent_id.is_(None),  # Only check parent complaints
        Complaint.city == city
    ).all()
...
def submit_complaint():
    ...
    if request.method == 'POST':
        ...
        if photo:
            ...
            photo_filename = f"{datetime.utcnow().strftime('%Y%m%d%H%M%S')}_{secure_filename(photo.filename)}"
            photo_path = os.path.join(app.config['UPLOAD_FOLDER'], photo_filename)
            photo.save(photo_path)
        
        geo = geolocator.geocode(f"{location}, {city}, {state}, India")
        lat, lon = (geo.latitude, geo.longitude) if geo else (None, None)
        
        similar = find_similar_complaints(lat, lon, title, description, city) if lat and lon else None
        ...
        if similar:
            complaint.parent_id = similar.id
            similar.is_thread = True
            similar.thread_count += 1
            
            # Notify the original complaint author
            send_notification(
                similar.user_id,
                "Similar Complaint Added",
                f"A similar complaint has been added to your thread: {title}",
                "thread_update",
                similar.id
            )
            
            # Notify users who voted on the original complaint
            for vote in similar.votes:
                if vote.user_id != session['user_id']:
                    send_notification(
                        vote.user_id,
                        "Update on Complaint You Voted On",
                        f"A similar complaint has been added to a thread you're following: {title}",
                        "thread_update",
                        similar.id
                    )
            
            flash("Similar complaint found! Your complaint has been added to an existing thread.")
        
        db.session.add(complaint)
        db.session.commit()
        
        # Notify department
        dept_contact = get_department_contact(city, department)
        if dept_contact:
            msg = Message(
            ...
            Please login to the system to respond to this complaint.
            """
            mail.send(msg)
        
        # Notify nearby complaint authors
        if lat and lon:
            nearby_complaints = Complaint.query.filter(
                Complaint.latitude.isnot(None),
                Complaint.longitude.isnot(None),
            ...
            for nearby in nearby_complaints:
                ...
                if distance <= 500 and nearby.user_id != session['user_id']:  # Within 500 meters
                    send_notification(
                        nearby.user_id,
                        "New Complaint in Your Area",
                        f"A new complaint has been reported near your complaint: {title}",
                        "nearby_complaint",
                        complaint.id
                    )
...
def dashboard():
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    # Get parent complaints and their threads
    complaints = Complaint.query.filter_by(parent_id=None).order_by(Complaint.timestamp.desc()).all()
...
def view_complaint(complaint_id):
    ...
    # If this is a threaded complaint, redirect to the parent
    if complaint.parent_id:
        return redirect(url_for('view_complaint', complaint_id=complaint.parent_id))
    
    # Get all responses for this complaint
    responses = complaint.responses.order_by(AuthorityResponse.timestamp.desc()).all()
    
    # Get all related complaints if this is a thread
    related_complaints = complaint.similar_complaints.all() if complaint.is_thread else []
